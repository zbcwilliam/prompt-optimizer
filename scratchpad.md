...
## 任务：修复历史记录实时更新和重用功能 - 2024-03-21

### 目标
修复历史记录不能实时更新和重用功能无法使用的问题。

### 问题分析
1. 历史记录不实时更新
   - 原因：添加新记录后没有更新响应式数据
   - 影响：需要刷新页面才能看到新记录
   - 修复：在添加记录后更新history.value

2. 重用功能无法使用
   - 原因：事件名称不匹配（select vs reuse）
   - 影响：点击重用按钮无效
   - 修复：统一使用reuse事件名

### 修复步骤
[X] 1. 修改App.vue中的历史记录更新逻辑
    - 在handleOptimizePrompt中添加history更新
    - 在handleIteratePrompt中添加history更新
    - 完成时间：2024-03-21
